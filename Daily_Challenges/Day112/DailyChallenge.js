...
// Intuition
// The goal of this algorithm is to find the maximum "beauty" of items a person can afford, given an array of items,
// where each item is represented as [price, beauty], and an array of queries, where each query represents an available budget.

// To do this efficiently:

// We preprocess the items to build a "price-beauty bracket" where each entry stores the highest possible beauty value achievable
...
// Sort the Items by Price: We first sort the items array by price in ascending order. Sorting helps ensure that each processed
// item has a price greater than or equal to previous items, allowing us to build a cumulative "price-beauty bracket."
...
// If the current item's beauty is higher than the last recorded beauty in res, we add a new bracket with the current item's
// price and beauty, updating the last bracket’s max_price to the current price.
// Result of Preprocessing: At the end of preprocessing, res will contain cumulative price-beauty brackets, allowing us to check
// the maximum beauty for any budget by examining the highest bracket we can afford.

// Part 2: Processing the Queries
// For each budget in queries, we iterate backward through res (from the most expensive bracket to the least) and find the first
// bracket with a minimum price that fits within the query budget.
